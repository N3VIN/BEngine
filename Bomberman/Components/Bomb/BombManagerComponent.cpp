...
void bomberman::BombManagerComponent::PlaceBomb(glm::ivec2 cell, bengine::GameObject *owner) {
    ...
    AddSprite(*bombGO, SpriteType::Bomb);
...
void bomberman::BombManagerComponent::DetonateBomb(BombComponent *bomb) {
    ...
    if (it != m_playerStats.end() && it->second.activeBombs > 0) {
        --it->second.activeBombs; // remove bomb
    }
    ...
    SpreadInDirection(cell, {0, -1}, radius, owner); // down
    SpreadInDirection(cell, {0, 1}, radius, owner); // up
    SpreadInDirection(cell, {-1, 0}, radius, owner); // left
    SpreadInDirection(cell, {1, 0}, radius, owner); // right
    ...
    for (int distance = 1; distance <= range; ++distance) {
        ...
        if (const auto *chainBombGO = BombAt(cell)) {
            if (auto *chainBomb = chainBombGO->GetComponent<BombComponent>()) {
                m_detonationQueue.push_back(chainBomb); // for chain reaction
            }
        ...
        const bool tipHere = distance == range || IsFlameBlocker(origin + direction * (distance + 1)); // end explosion sprite
...
void bomberman::BombManagerComponent::SpawnFlame(glm::ivec2 cell, const SpriteDefinition &piece) const {
    ...
    AddSprite(*flameGO)->Play(piece, true, GetTileset().animationFps);
