...
void bomberman::ModeSelectState::OnEnter() {
    ...
    CreateMenuLabel(*m_scene, "SELECT MODE", textSize::title, bengine::colors::white, bengine::ScreenFraction(0.5f, 0.25f));
    ...
    const auto launch = [](GameMode mode) {
        bengine::ServiceLocator::GetAudioService().StopAudio(bengine::Hash("title_screen"));
    ...
    menu->AddItem(CreateMenuLabel(*m_scene, "Solo", textSize::body, bengine::colors::menuItem, bengine::ScreenFraction(0.5f, startY)),
                  [launch] {
                      launch(GameMode::Solo);
                  }
    );

    menu->AddItem(CreateMenuLabel(*m_scene, "COOP", textSize::body, bengine::colors::menuItem, bengine::ScreenFraction(0.5f, startY + stepY)),
                  [launch] {
                      launch(GameMode::Coop);
                  }
    );

    menu->AddItem(CreateMenuLabel(*m_scene, "Versus", textSize::body, bengine::colors::menuItem, bengine::ScreenFraction(0.5f, startY + stepY * 2.0f)),
                  [launch] {
                      launch(GameMode::Versus);
                  }
    );

    menu->AddItem(CreateMenuLabel(*m_scene, "Back", textSize::body, bengine::colors::menuItem, bengine::ScreenFraction(0.5f, startY + stepY * 3.0f)),
                  [] {
